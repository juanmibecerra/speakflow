...
const appStyles = `
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .grammar-example { font-size: 13px; padding: 4px 0; color: var(--text); }
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (data.lastDate !== today) {
        const newData = { lastDate: today, count: streak };
        localStorage.setItem('speakflow_streak', JSON.stringify(newData));
      }
    } catch { /* ignore */ }
  }, [streak]);
...
function LessonPage() {
  ...
  const playAll = () => {
    ...
    lesson.dialogue.forEach((line, i) => {
      setTimeout(() => playLine(line.text, i), delay);
...
function FlashcardsPage() {
  ...
  const rate = (difficulty) => {
    ...
    if (cardIdx + 1 >= cards.length) {
      ...
      try {
        const prev = JSON.parse(localStorage.getItem('speakflow_flashcards') || '{}');
        const today = getToday();
        prev[today] = { total: cards.length, easy: newResults.filter(r => r.difficulty === 'easy').length, ok: newResults.filter(r => r.difficulty === 'ok').length, hard: newResults.filter(r => r.difficulty === 'hard').length };
        localStorage.setItem('speakflow_flashcards', JSON.stringify(prev));
      } catch { /* ignore */ }
    } else {
...
function TutorPage() {
  ...
  const [apiKey, setApiKey] = useState(() => {
    try { return localStorage.getItem('speakflow_api_key') || ''; } catch { return ''; }
  });
  ...
  const hasFunction = typeof window !== 'undefined';

  const saveApiKey = (key) => {
    setApiKey(key);
    try { localStorage.setItem('speakflow_api_key', key); } catch {}
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      try {
        const fnResponse = await fetch('/.netlify/functions/chat', {
          method: 'POST',
        ...
        // Netlify function not available, fall through
      }
      ...
      // Fallback: local smart tutor (no API needed)
      if (!reply) {
        reply = getLocalTutorReply(userMsg);
      }

      setMessages(prev => [...prev, { role: 'assistant', content: reply }]);
    } catch {
      setMessages(prev => [...prev, { role: 'assistant', content: getLocalTutorReply(userMsg) }]);
    }
    setLoading(false);
  };
    ...
    <div className="section-wrap">
      ...
      {!apiKey && (
        <div style={{marginBottom:'12px'}}>
          <button className="btn btn-sm btn-secondary" onClick={() => setShowKeySetup(!showKeySetup)} style={{fontSize:'11px', width:'100%'}}>
            ⚙️ {showKeySetup ? 'Ocultar configuración' : 'Configurar API Key de Anthropic (opcional — funciona sin ella)'}
          </button>
          {showKeySetup && (
            <div className="api-key-setup">
              <p style={{fontSize:'12px', color:'var(--text2)', lineHeight:'1.5'}}>
                Para respuestas con IA avanzada, introduce tu API key de Anthropic. Sin ella, funciona un tutor local con correcciones básicas.
              </p>
              <input
                type="password"
                placeholder="sk-ant-..."
...
function getLocalTutorReply(userMsg) {
  ...
  if (/\bi am agree\b/.test(lower)) corrections.push('✏️ Small fix: "I agree" (not "I am agree" — agree is already a verb)');
  if (/\bi have \d+ years\b/.test(lower) && !/old/.test(lower)) corrections.push('✏️ Small fix: "I am X years old" (use "be", not "have" for age in English)');
  if (/\bhe don\'t\b|\bshe don\'t\b/.test(lower)) corrections.push('✏️ Small fix: "he/she doesn\'t" (third person uses "doesn\'t")');
  if (/\bI goed\b|\bI goed\b/i.test(lower)) corrections.push('✏️ Small fix: "I went" (go → went, irregular verb)');
  if (/\bmore better\b/.test(lower)) corrections.push('✏️ Small fix: just "better" (it\'s already comparative, no need for "more")');
  if (/\bpeoples\b/.test(lower)) corrections.push('✏️ Small fix: "people" (already plural, "peoples" means different ethnic groups)');
  if (/\binformations\b/.test(lower)) corrections.push('✏️ Small fix: "information" (uncountable in English)');
  if (/\bI am working since\b/.test(lower)) corrections.push('✏️ Small fix: "I have been working since..." (use Present Perfect Continuous for duration)');
  ...
  if (corrections.length > 0) {
    ...
    response = '✅ Good sentence!\n\n';
  }

  // Topic-based responses
  if (/weekend|saturday|sunday/i.test(lower)) {
    response += "Sounds interesting! What do you usually enjoy doing on weekends? Do you prefer staying in or going out?\n\n💡 New expression: \"to have a lie-in\" = to stay in bed late (tener un día de quedarse en la cama)";
  } else if (/interview|job|work/i.test(lower)) {
    response += "That's a great topic to practice! Can you tell me about your current role? What are your main responsibilities?\n\n💡 New expression: \"to wear many hats\" = to have many different roles (tener muchos roles)";
  } else if (/food|eat|restaurant|cook/i.test(lower)) {
    response += "I love talking about food! What's your favourite dish to cook? Or do you prefer eating out?\n\n💡 New expression: \"to grab a bite\" = to eat something quickly (comer algo rápido)";
  } else if (/travel|holiday|trip|vacation/i.test(lower)) {
    response += "Travelling is wonderful for learning! Where was the last place you visited? What did you enjoy most?\n\n💡 New expression: \"off the beaten track\" = away from popular tourist areas (fuera de las rutas turísticas)";
  } else if (/hello|hi|hey|good morning|good afternoon/i.test(lower)) {
    response += "Hello! Great to chat with you. What would you like to talk about today? We can practice any topic you'd like.\n\n💡 New expression: \"How's it going?\" = informal greeting, like ¿Qué tal?";
  } else if (/phrasal|verb|get|take|put|come/i.test(lower)) {
    response += "Phrasal verbs are super important in everyday English. Here are some useful ones:\n• get along with — llevarse bien\n• get over — superar\n• get by — arreglárselas\n• get around to — encontrar tiempo para\n\nCan you make a sentence with one of them?";
  } else {
    const followUps = [
      "That's interesting! Can you tell me more about that? Try to use at least two sentences.",
      "Great effort! Can you give me an example or explain why?",
      "I see! What's your opinion on this? Try using 'I think that...' or 'In my view...'",
      "Nice! Can you rephrase that using different words? It's great practice for vocabulary.",
    ];
